...
import {
  ...
  Calendar, MessageSquare, ChevronRight, Clock, TrendingUp, Award, Newspaper,
} from "lucide-react";
...
const Dashboard = () => {
  ...
  const displayName = profile?.full_name?.split(" ")[0] || "Aspirant";
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container py-6 md:py-8">
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="visible" className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-2xl font-bold md:text-3xl">Good morning, {displayName}</h1>
          ...
          <div className="hidden md:flex items-center gap-4">
            <div className="flex items-center gap-2 rounded-sm bg-accent/10 px-3 py-1.5">
              ...
              <span className="font-mono-data text-sm font-bold text-accent">{profile?.streak_days || 0} day streak</span>
            </div>
          </div>
        </motion.div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
          {[
            ...
            <motion.div key={stat.label} variants={fadeUp} initial="hidden" animate="visible" transition={{ delay: i * 0.1 }}
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2 rounded-lg border border-border bg-card p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-display text-lg font-semibold flex items-center gap-2">
                <Calendar className="h-5 w-5 text-accent" /> Today's Schedule
              </h2>
          ...
          <div className="rounded-lg border border-border bg-card p-6">
            <h2 className="font-display text-lg font-semibold flex items-center gap-2 mb-4">
              <Users className="h-5 w-5 text-accent" /> Study Circle
            </h2>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-3">
          <motion.div variants={fadeUp} initial="hidden" animate="visible" transition={{ delay: 0.2 }}
            ...
            <h2 className="font-display text-lg font-semibold flex items-center gap-2 mb-3">
              <Brain className="h-5 w-5 text-accent" /> AI Tutor
            </h2>
            ...
            <Button variant="hero" size="sm" className="w-full" asChild>
              <Link to="/ai-tutor">Ask AI Tutor <ChevronRight className="ml-1 h-4 w-4" /></Link>
            </Button>
          </motion.div>

          <div className="lg:col-span-2 rounded-lg border border-border bg-card p-6">
            <h2 className="font-display text-lg font-semibold flex items-center gap-2 mb-4">
              <BarChart3 className="h-5 w-5 text-accent" /> Recent Tests
            </h2>
            ...
            <Button variant="outline" size="sm" className="mt-4 w-full" asChild>
              <Link to="/mock-tests">View All Tests <ChevronRight className="ml-1 h-4 w-4" /></Link>
            </Button>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-5">
          {[
            { icon: FileEdit, label: "Write Answer", color: "text-accent", href: "/answer-writing" },
            { icon: Target, label: "Take Mock Test", color: "text-primary", href: "/mock-tests" },
            { icon: Newspaper, label: "Current Affairs", color: "text-success", href: "/current-affairs" },
            { icon: Brain, label: "AI Tutor", color: "text-warning", href: "/ai-tutor" },
            { icon: BookOpen, label: "Study Circle", color: "text-accent", href: "/dashboard" },
          ].map((action, i) => (
            <motion.div key={action.label} variants={fadeUp} initial="hidden" animate="visible" transition={{ delay: i * 0.05 }}>
              <Link to={action.href}
